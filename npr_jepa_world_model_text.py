...
def train_cycle(model, cache, s2i, i2s, chains, tokenizer, all_sv, n_iters, cycle, lr):
    ...
    for it in range(n_iters):
        ...
        loss = (lce + 0.2 * latent_loss + 0.1 * div_loss(prog) + 0.05 * use_loss(prog, np_) + 0.1 * novelty_loss(prog,
                ...
                cfg["action_loss_weight"] * al_loss + cfg["precondition_loss_weight"] * precond_loss) / cfg[
                   "grad_accumulation"]
        loss.backward()
        ...
        if it % 300 == 0:
            ...
            with torch.no_grad():
                ...
                pw = i2s.get(lg.argmax().item(), "?");
                tw = t["expected"]
                st = t["state"]
                ostr = fmt_attn(oa2, tokenizer, f" {st}")
            ms = f" [D{t['depth']}]" if t.get("task_type") == "multistep" else ""
            print(
                f"  [C{cycle}] {it:4d}/{n_iters} | WM:{lce.item():.2f} L:{latent_loss.item():.2f} G:{gl.item():.2f} A:{al_loss.item():.2f} P:{precond_loss.item():.2f} | "
...
def evaluate(model, cache, s2i, i2s, chains, tokenizer):
    ...
    with torch.no_grad():
        for rule in sorted(TRANSITION_RULES.keys()):
            ...
            st = t["state"];
            prop = TRANSITION_RULES[rule]["property_changed"]
        ...
        for rule in sorted(TRANSITION_RULES.keys()):
            ...
            for _ in range(ne):
                ...
                if shown < 1:
                    st = t["state"]
                    print(
                        f"    [{rule:12s}] '{st[:18]}' + '{t['action']:9s}' -> '{pred[:22]}' {'V' if pred == target else 'X'}")
    ...
    ov3 = sum(t for _, t in results.values()) / n
    ...
    for prop, slots in sorted(prop_slots.items()):
        ...
        print(
            f"    {prop:15s}: dominant slot = S{dominant[0]} ({dominant[1]}/{len(slots)} = {100 * dominant[1] / len(slots):.0f}%)")
    ...
    if CHAIN_TRIPLES:
        ...
        if ce_tested:
            print(
                f"    CHAIN EFFECTS: {100 * ce_ok / ce_tested:.0f}% (top3: {100 * ce_top3 / ce_tested:.0f}%) [{ce_tested} tested]")
        ...
        with torch.no_grad():
            for rname in graded_exist:
                ...
                for s, a, res in r["triples"]:
                    ...
                    if gr_tested <= 6:
                        print(
                            f"    [{rname:10s}] '{t['state'][:20]}' + '{t['action'][:8]}' -> '{pred[:22]}' {'V' if correct else 'X'}")
...
def main():
    ...
    with torch.no_grad():
        for _ in range(200):
            ...
            for sel in pr: usage[
                model.lib.names[sel.argmax().item()] if sel.argmax().item() < len(model.lib.names) else "?"] += 1
    tu = sum(usage.values())
