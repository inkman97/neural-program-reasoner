...
class PrimitiveLibrary(nn.Module):
    def __init__(self, sd):
        ...
        self.negate = nn.Sequential(nn.Linear(sd, sd * 2), nn.GELU(), nn.Linear(sd * 2, sd * 2), nn.GELU(),
                                    nn.Linear(sd * 2, sd))
        self.morph = nn.Sequential(nn.Linear(sd, sd), nn.LayerNorm(sd), nn.GELU(), nn.Linear(sd, sd))
        self.aq, self.ak, self.av, self.ao = nn.Linear(sd, sd // 4), nn.Linear(sd, sd // 4), nn.Linear(sd,
                                                                                                       sd), nn.Linear(
            sd, sd)
...
def probe_primitives(model, cache, relations, w2i):
    ...
    with torch.no_grad():
        ...
        print(
            f"  3. IDENTITY passthrough: {sum((lib.apply(0, v) - v).norm().item() / max(v.norm().item(), 1e-8) for v in vecs[:50]) / 50:.4f}")
        ...
        for i in range(np_):
            print(f"    {lib.names[i]:12s}" + "".join(
                f" {F.cosine_similarity(ad[i].unsqueeze(0), ad[j].unsqueeze(0)).item():6.2f}" for j in range(np_)))
    print("  === END PROBING ===\n")
...
class Synthesizer(nn.Module):
    def __init__(self, sd, np, max_steps, pd=256):
        ...
        self.heads = nn.ModuleList(
            [nn.Sequential(nn.Linear(pd, 128), nn.GELU(), nn.Linear(128, np)) for _ in range(max_steps)])
...
class Generator(nn.Module):
    def __init__(self, sd, pd, vs):
        super().__init__()
        self.h = nn.Sequential(nn.Linear(sd * 2 + pd, sd), nn.LayerNorm(sd), nn.GELU(), nn.Dropout(0.1),
...
class NPR(nn.Module):
    def __init__(self, state_dim, vocab_size):
        ...
        self.adapt = nn.Sequential(nn.Linear(state_dim, state_dim), nn.LayerNorm(state_dim), nn.GELU(),
...
def train_cycle(model, cache, w2i, i2w, rels, comp_tasks, n_iters, cycle, lr):
    ...
    for it in range(n_iters):
        ...
        loss = (lce + 0.1 * div_loss(prog) + 0.05 * use_loss(prog, np_) + 0.1 * novelty_loss(prog,
                                                                                             np_) + 0.03 * stop_l + 0.02 * len(
            prog)) / cfg["grad_accumulation"]
        loss.backward()
        ...
        if it % 500 == 0:
            ...
            print(
                f"  [C{cycle}] {it:5d}/{n_iters} | L:{lce.item():.3f} | Acc:{acc:5.1f}% Comp:{cacc:4.0f}% | {speed:.0f}it/s | "
...
def evaluate(model, cache, w2i, i2w, rels, comp_tasks=None, gpt2=None, tokenizer=None):
    ...
    if n:
        ...
        if gpt2: print(
            f"    GPT2 OVERALL: {sum(a for a, _ in g2_res.values()) / n:.1f}% (top3: {sum(t for _, t in g2_res.values()) / n:.1f}%)")
            ...
            for cn in sorted(comp_tasks.keys()):
                ...
                for _ in range(nt):
                    ...
                    if shown < 2:
                        ...
                        print(
                            f"    {task['test_word']} -> [{inter}] -> {target} | pred:{pred} {'V' if pred == target else 'X'} K={len(pr)} D={depth}")
...
def main():
    ...
    for c in range(cfg["num_cycles"]):
        print(f"{'=' * 65}")
        print(
            f"CYCLE {c + 1}/{cfg['num_cycles']} | Prims: {model.lib.n} | Params: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
