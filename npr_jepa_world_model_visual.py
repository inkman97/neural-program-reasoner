...
# === WORLD DEFINITION ===
OBJECTS = {"water": {"temperature": True}, "soup": {"temperature": True}, "coffee": {"temperature": True}, "tea": {"temperature": True}, "milk": {"temperature": True}, "food": {"temperature": True}, "oil": {"temperature": True}, "pan": {"temperature": True}, "iron": {"temperature": True}, "metal": {"temperature": True}, "door": {"openness": True}, "window": {"openness": True}, "box": {"openness": True}, "jar": {"openness": True}, "gate": {"openness": True}, "drawer": {"openness": True}, "lid": {"openness": True}, "bag": {"openness": True}, "bottle": {"openness": True}, "cabinet": {"openness": True}, "lamp": {"power": True, "surface": "table", "fragile": True, "integrity": True}, "light": {"power": True}, "screen": {"power": True}, "tv": {"power": True}, "radio": {"power": True}, "computer": {"power": True}, "fan": {"power": True}, "heater": {"power": True}, "oven": {"power": True}, "speaker": {"power": True}, "cup": {"fullness": True, "surface": "shelf", "fragile": True, "integrity": True}, "glass": {"fullness": True, "surface": "table", "fragile": True, "integrity": True}, "bowl": {"fullness": True, "fragile": True, "integrity": True}, "pot": {"fullness": True}, "tub": {"fullness": True}, "bucket": {"fullness": True}, "jug": {"fullness": True}, "tank": {"fullness": True}, "pool": {"fullness": True}, "plate": {"integrity": True, "surface": "counter", "fragile": True}, "vase": {"integrity": True, "fragile": True, "surface": "shelf"}, "mirror": {"integrity": True}, "ball": {"surface": "table"}, "book": {"surface": "desk"}, "phone": {"surface": "bed"}, "toy": {"surface": "shelf"}, "pen": {"surface": "desk"}, "remote": {"surface": "couch"}, "key": {"container": "pocket"}, "coin": {"container": "jar"}, "letter": {"container": "envelope"}, "shirt": {"container": "closet"}, "tool": {"container": "shed"}, "food_c": {"container": "fridge"}, "ball_c": {"container": "box"}, "toy_c": {"container": "bag"}, "pen_c": {"container": "case"}, "book_c": {"container": "drawer"}}
ACTIONS = {"heat": {"property": "temperature", "from": "cold", "to": "hot"}, "cool": {"property": "temperature", "from": "hot", "to": "cold"}, "warm up": {"property": "temperature", "from": "cold", "to": "warm"}, "boil": {"property": "temperature", "from": "hot", "to": "boiling"}, "simmer": {"property": "temperature", "from": "boiling", "to": "hot"}, "open": {"property": "openness", "from": "closed", "to": "open"}, "close": {"property": "openness", "from": "open", "to": "closed"}, "switch on": {"property": "power", "from": "off", "to": "on"}, "switch off": {"property": "power", "from": "on", "to": "off"}, "fill": {"property": "fullness", "from": "empty", "to": "full"}, "drop": {"property": "integrity", "from": "intact", "to": "broken"}, "push": {"property": "position", "from": "surface", "to": "floor"}, "put in": {"property": "containment", "from": "outside", "to": "inside"}}

def generate_world_observations():
    observations = []; rule_groups = defaultdict(list)
    for obj_name, obj_props in OBJECTS.items():
        for act_name, act_def in ACTIONS.items():
            ...
            if prop == "position": state = f"the {display} is on the {obj_props['surface']}"; result = f"the {display} is on the floor"
            elif prop == "containment": state = f"the {display} is outside the {obj_props['container']}"; result = f"the {display} is inside the {obj_props['container']}"
            else: state = f"the {display} is {act_def['from']}"; result = f"the {display} is {act_def['to']}"
            obs = {"state": state, "action": act_name, "result": result, "property": prop, "object": display, "fragile": obj_props.get("fragile", False)}
            observations.append(obs); rule_groups[act_name].append(obs)
...
for act_name, obs_list in RULE_GROUPS.items():
    ...
    if triples: TRANSITION_RULES[act_name] = {"triples": triples, "precondition": ACTIONS[act_name]["from"].split()[0], "property_changed": obs_list[0]["property"]}
...
for ce in CHAIN_EFFECTS:
    ...
    for obs in RULE_GROUPS.get(ce["trigger_action"], []):
        if obs.get(ce["condition"], False) and obs["object"] in chain_by_obj:
            co = chain_by_obj[obs["object"]]; CHAIN_TRIPLES.append({"state": obs["state"], "action": obs["action"], "result": co["result"], "intermediate": obs["result"], "rules": [ce["trigger_action"], ce["chain_action"]], "object": obs["object"]})
...
def build_vocab():
    ...
    sl = sorted(ss); return sl, {s: i for i, s in enumerate(sl)}, {i: s for i, s in enumerate(sl)}
...
def find_chains(max_len=3):
    ...
    print(f"Chains: d1:{len(chains[1])}, d2:{len(chains[2])}, d3:{len(chains[3])}"); return chains
...
BG_COLORS = {
    "temperature": {"cold": (200,220,255), "warm": (255,240,200), "hot": (255,210,200), "boiling": (240,200,240)},
...
def render_state_image(state_text, size=224):
    ...
    prop_val = ""
    prop_type = ""
    if "on the floor" in state_text: prop_val = "floor"; prop_type = "position"
    elif "on the" in state_text: prop_val = "surface"; prop_type = "position"
    elif "outside" in state_text: prop_val = "outside"; prop_type = "containment"
    elif "inside" in state_text: prop_val = "inside"; prop_type = "containment"
    else:
        for ptype, vals in BG_COLORS.items():
            for val in vals:
                if f"is {val}" in state_text:
                    prop_val = val; prop_type = ptype; break
            if prop_val: break
    ...
    if prop_val == "floor": cy = floor_y - 15  # on the ground
    elif prop_val == "surface": cy = floor_y - 65  # elevated

    # Draw based on object type and property
    h = _hash(obj_name)

    # TEMPERATURE objects (liquids/metals)
    if prop_type == "temperature" or (obj_name in ["water","soup","coffee","tea","milk","food","oil","pan","iron","metal"]):
        ...
        lw = obj_sz - 10
        lh = obj_sz//2
        if prop_val == "cold": lc = (100, 150, 255)
        elif prop_val == "warm": lc = (255, 200, 100)
        elif prop_val == "hot": lc = (255, 80, 50)
    ...
    marker_shape = h % 4
    if marker_shape == 0: draw.ellipse([mx-8, my-8, mx+8, my+8], fill=marker_color, outline=(0,0,0))
    elif marker_shape == 1: draw.rectangle([mx-8, my-8, mx+8, my+8], fill=marker_color, outline=(0,0,0))
    elif marker_shape == 2: draw.polygon([(mx, my-8), (mx+8, my+8), (mx-8, my+8)], fill=marker_color, outline=(0,0,0))
    else: draw.polygon([(mx, my-8), (mx+8, my), (mx, my+8), (mx-8, my)], fill=marker_color, outline=(0,0,0))
...
class ImageEmbeddingCache:
    ...
    def _render_action(self, act_name, size=224):
        ...
        img = Image.new('RGB', (size,size), (240,240,240)); draw = ImageDraw.Draw(img)
        h = _hash(act_name)
        r,g,b = (h*37)%200+55, (h*73)%200+55, (h*113)%200+55
        cx,cy = size//2, size//2
        # Large directional arrow
        draw.polygon([(cx-50,cy-20),(cx+30,cy-20),(cx+30,cy-40),(cx+70,cy),(cx+30,cy+40),(cx+30,cy+20),(cx-50,cy+20)],
    ...
    def get(self, state_text):
        ...
        self.cache[state_text] = vec.clone(); return vec

    def get_action(self, action_name): return self.get(f"__action__{action_name}")
    ...
    def get_tokens(self, state_text):
        ...
        self.token_cache[state_text] = tokens.clone(); return tokens


# === MODEL COMPONENTS (v2 — no projection head) ===
class SlotAttention(nn.Module):
    def __init__(self, sd, num_slots=2, n_iters=2):
        super().__init__(); self.num_slots=num_slots; self.n_iters=n_iters; self.sd=sd
        self.slot_mu=nn.Parameter(torch.randn(num_slots,sd)*0.02); self.k_proj=nn.Linear(sd,sd); self.v_proj=nn.Linear(sd,sd); self.q_proj=nn.Linear(sd,sd)
        self.gru=nn.GRUCell(sd,sd); self.mlp=nn.Sequential(nn.Linear(sd,sd*2),nn.GELU(),nn.Linear(sd*2,sd)); self.norm1=nn.LayerNorm(sd); self.norm2=nn.LayerNorm(sd); self.scale=math.sqrt(sd)
    def forward(self, token_reprs):
        k=self.k_proj(token_reprs); v=self.v_proj(token_reprs); slots=self.slot_mu.clone()
        for _ in range(self.n_iters):
            sp=slots; slots=self.norm1(slots); q=self.q_proj(slots); al=torch.matmul(q,k.T)/self.scale
            attn=F.softmax(al,dim=0); aw=attn/(attn.sum(dim=-1,keepdim=True)+1e-8); updates=torch.matmul(aw,v)
            slots=self.gru(updates.reshape(-1,self.sd),sp.reshape(-1,self.sd)); slots=slots.reshape(self.num_slots,self.sd); slots=slots+self.mlp(self.norm2(slots))
        return slots, attn

class ObjectExtractor(nn.Module):
    def __init__(self, sd):
        super().__init__(); self.query=nn.Parameter(torch.randn(sd)*0.02); self.k=nn.Linear(sd,sd); self.v=nn.Linear(sd,sd)
        self.out=nn.Sequential(nn.Linear(sd,sd),nn.LayerNorm(sd)); self.scale=math.sqrt(sd)
    def forward(self, tokens):
        k,v=self.k(tokens),self.v(tokens); a=F.softmax(torch.matmul(k,self.query)/self.scale,dim=0)
...
class PrimitiveLibrary(nn.Module):
    def __init__(self, sd):
        super().__init__(); self.sd=sd; self.names=list(BASE_NAMES)
        self.identity=nn.Sequential(nn.Linear(sd,sd),nn.Tanh()); self.negate_v=nn.Parameter(torch.randn(sd)*0.1)
        self.morph=nn.Sequential(nn.Linear(sd,sd),nn.LayerNorm(sd),nn.GELU(),nn.Linear(sd,sd))
        self.aq=nn.Linear(sd,sd//4); self.ak=nn.Linear(sd,sd//4); self.av=nn.Linear(sd,sd); self.ao=nn.Linear(sd,sd)
        self.lookup=nn.Sequential(nn.Linear(sd,sd*2),nn.GELU(),nn.Linear(sd*2,sd*2),nn.GELU(),nn.Linear(sd*2,sd))
        self.bc=nn.Parameter(torch.randn(sd)*0.01); self.bn=nn.Sequential(nn.Linear(sd*2,sd),nn.GELU(),nn.Linear(sd,sd))
        self.gates=nn.ParameterList([nn.Parameter(torch.tensor(g)) for g in [0.01,0.5,0.3,0.4,0.5,0.3]])
        self.inv_p=nn.ModuleList(); self.inv_g=nn.ParameterList()
    @property
    def n(self): return len(BASE_NAMES)+len(self.inv_p)
    def _householder(self, x):
        v=self.negate_v; return x-2*(torch.dot(v,x)/(torch.dot(v,v)+1e-8))*v
    def _base(self, i, s):
        ...
        if i==3:
            q,k,v=self.aq(s),self.ak(s),self.av(s); return s+torch.sigmoid(self.gates[3])*self.ao(torch.sigmoid(torch.dot(q,k)/math.sqrt(q.shape[0]))*v)
    ...
    def add(self, name, a, b):
        ...
        self.inv_g.append(nn.Parameter(torch.tensor(0.4,device=dev))); self.names.append(name); print(f"  [COMPRESS] Created: {name}"); return self.n-1

class SlotSelector(nn.Module):
    def __init__(self, ad, ns):
        super().__init__(); self.net=nn.Sequential(nn.Linear(ad,128),nn.GELU(),nn.Linear(128,ns))
    def forward(self, a): return F.softmax(self.net(a),dim=-1)

class PropertyUpdater(nn.Module):
    def __init__(self, sd, ad, ms):
        super().__init__(); self.step_emb=nn.Embedding(ms,sd); self.action_film=nn.Sequential(nn.Linear(ad,sd*2),nn.GELU(),nn.Linear(sd*2,sd*2)); self.sd=sd
    def forward(self, pv, action, prog, lib):
        film=self.action_film(action); scale,shift=film[:self.sd],film[self.sd:]
        for i,sel in enumerate(prog):
            pv=pv+self.step_emb(torch.tensor(i,device=pv.device)); pv=pv*(1+0.1*torch.tanh(scale))+0.1*torch.tanh(shift); pv=lib.apply_soft(sel,pv)
        return pv

class PreconditionChecker(nn.Module):
    def __init__(self, sd, ad):
        super().__init__(); self.net=nn.Sequential(nn.Linear(sd+ad,sd//2),nn.GELU(),nn.Linear(sd//2,1),nn.Sigmoid())
    def forward(self, s, a): return self.net(torch.cat([s,a]))

class RuleSynthesizer(nn.Module):
    def __init__(self, ed, sd, np_, ms, pd=256):
        super().__init__(); self.pd=pd; self._np=np_
        self.proj=nn.Sequential(nn.Linear(ed,pd),nn.LayerNorm(pd),nn.GELU())
        self.a1=nn.MultiheadAttention(pd,4,batch_first=True); self.n1=nn.LayerNorm(pd); self.n2=nn.LayerNorm(pd)
        self.f1=nn.Sequential(nn.Linear(pd,pd*2),nn.GELU(),nn.Linear(pd*2,pd))
        self.a2=nn.MultiheadAttention(pd,4,batch_first=True); self.n3=nn.LayerNorm(pd); self.n4=nn.LayerNorm(pd)
        self.f2=nn.Sequential(nn.Linear(pd,pd*2),nn.GELU(),nn.Linear(pd*2,pd))
        self.heads=nn.ModuleList([nn.Sequential(nn.Linear(pd,128),nn.GELU(),nn.Linear(128,np_)) for _ in range(ms)])
        self.stop=nn.Sequential(nn.Linear(pd+sd,128),nn.GELU(),nn.Linear(128,1),nn.Sigmoid())
    def signature(self, ex):
        x=self.proj(ex).unsqueeze(0); a,_=self.a1(x,x,x); x=self.n1(x+a); x=self.n2(x+self.f1(x))
        a,_=self.a2(x,x,x); x=self.n3(x+a); x=self.n4(x+self.f2(x)); return x.mean(1).squeeze(0)
    def rebuild(self, new_n):
        ...
        for h in self.heads:
            ...
            with torch.no_grad():
                n[0].weight.copy_(h[0].weight); n[0].bias.copy_(h[0].bias); n[2].weight[:old].copy_(h[2].weight); n[2].bias[:old].copy_(h[2].bias)
                aw=h[2].weight.mean(0,keepdim=True).expand(new_n-old,-1); n[2].weight[old:].copy_(aw+torch.randn_like(aw)*0.05); n[2].bias[old:].fill_(h[2].bias[:old].mean().item())
            nh.append(n)
        self.heads=nh; self._np=new_n
    def forward(self, ex, state, temp=0.8, np_=None, min_steps=2):
        pat=self.signature(ex); np_=np_ or self._np; prog=[]; stop_probs=[]
...
class Memory:
    def __init__(self, cap=300): self.cap=cap; self.entries=[]
    ...
    def stats(self): return f"{len(self.entries)} entries, {sum(1 for e in self.entries if e['ok'])} correct" if self.entries else "Empty"

class GoalEvaluator(nn.Module):
    def __init__(self, sd):
        super().__init__(); self.net=nn.Sequential(nn.Linear(sd*2,sd),nn.LayerNorm(sd),nn.GELU(),nn.Linear(sd,sd//2),nn.GELU(),nn.Linear(sd//2,1),nn.Sigmoid())
...
class JEPAWorldModel(nn.Module):
    def __init__(self, sd):
        super().__init__(); cfg=CONFIG; ed=sd*3; ns=cfg["num_slots"]; self.sd=sd
        self.obj_ext=ObjectExtractor(sd).to(DEVICE); self.slot_attn=SlotAttention(sd,ns).to(DEVICE)
        self.lib=PrimitiveLibrary(sd).to(DEVICE); self.syn=RuleSynthesizer(ed,sd,len(BASE_NAMES),cfg["max_program_steps"],cfg["proj_dim"]).to(DEVICE)
        self.slot_selector=SlotSelector(sd,ns).to(DEVICE); self.prop_updater=PropertyUpdater(sd,sd,cfg["max_program_steps"]).to(DEVICE)
        self.precond=PreconditionChecker(sd,sd).to(DEVICE); self.latent_pred=LatentPredictor(sd,sd,cfg["proj_dim"]).to(DEVICE)
        self.goal_eval=GoalEvaluator(sd).to(DEVICE)
        self.mem=Memory(cfg["memory_capacity"])

    def forward(self, task, temp=0.8, use_mem=True):
        ex=task["example_reprs"].to(DEVICE); ta=task["test_action"].to(DEVICE); tt=task["test_state_tokens"].to(DEVICE)
        obj_vec,oa=self.obj_ext(tt); slots,sa=self.slot_attn(tt)
        sw=self.slot_selector(ta); pv=(sw.unsqueeze(1)*slots).sum(0)
        ts=task["test_state"].to(DEVICE); pc=self.precond(ts,ta)
        np_=self.lib.n; sig=self.syn.signature(ex); from_mem=False; prog=None; stop_probs=[]
        ...
        if task.get("task_type")=="multistep" and "test_actions" in task:
            p=pv; pse=task.get("per_step_examples")
            for si,a in enumerate(task["test_actions"]):
                ad=a.to(DEVICE)
                if pse and si<len(pse): sp,_,_=self.syn(pse[si].to(DEVICE),p,temp,np_)
                else: sp=prog
                p=self.prop_updater(p,ad,sp,self.lib)
            transformed=p
        else: transformed=self.prop_updater(pv,ta,prog,self.lib)
...
def generate_single_task(cache, s2i, num_examples=5, rule_name=None, rules_dict=None):
    ...
    return {"example_reprs":torch.stack([torch.cat([cache.get(s),cache.get_action(a),cache.get(res)]) for s,a,res in examples]),
            ...
            "target_vec":cache.get(test[2]),"rule_name":rule_name,"state":test[0],"action":test[1],"expected":test[2],
            "depth":1,"task_type":"single","result_state_vec":cache.get(test[2]),"action_idx":ACT2IDX[test[1]],
            "precondition":r["precondition"],"property_changed":r["property_changed"],"precondition_holds":r["precondition"] in test[0],"property_idx":PROP2IDX[r["property_changed"]]}
...
def generate_multistep_task(cache, s2i, chains, num_examples=3):
    ...
    depth=random.choice(avail); pool=chains[depth]; ne=min(num_examples,len(pool)-1)
    chosen=random.sample(pool,ne+1); examples,test=chosen[:ne],chosen[ne]
    er=[torch.cat([cache.get(c['states'][0]),torch.stack([cache.get_action(a) for a in c["actions"]]).mean(0),cache.get(c['states'][-1])]) for c in examples]
    ta=[cache.get_action(a) for a in test["actions"]]
    pse=[]
    for rn in test["rules"]:
        r=TRANSITION_RULES[rn]; st=random.sample(r["triples"],min(3,len(r["triples"])))
    ...
    return {"example_reprs":torch.stack(er),"test_state":cache.get(test['states'][0]),"test_action":torch.stack(ta).mean(0),"test_actions":ta,
            "test_state_tokens":cache.get_tokens(test['states'][0]),"target_vec":cache.get(test['states'][-1]),
            "rule_name":"+".join(test["rules"]),"state":test["states"][0],"action":"+".join(test["actions"]),"expected":test["states"][-1],
            "depth":depth,"task_type":"multistep","result_state_vec":cache.get(test['states'][-1]),"action_idx":ACT2IDX[test["actions"][0]],
...
def generate_chain_effect_task(cache, s2i):
    ...
    ct=random.choice(CHAIN_TRIPLES); ne=min(3,len(CHAIN_TRIPLES)-1)
    ...
    return {"example_reprs":torch.stack(er),"test_state":cache.get(ct['state']),"test_action":cache.get_action(ct['action']),
            "test_state_tokens":cache.get_tokens(ct['state']),"target_vec":cache.get(ct['result']),
            "rule_name":f"chain:{'+'.join(ct['rules'])}","state":ct['state'],"action":ct['action'],"expected":ct['result'],
            "depth":1,"task_type":"chain","result_state_vec":cache.get(ct['result']),"action_idx":ACT2IDX[ct['action']],
            "precondition":"on","property_changed":"position","precondition_holds":True,"property_idx":PROP2IDX["position"]}
...
def div_loss(prog):
    ...
    l=torch.tensor(0.0,device=DEVICE); n=len(prog)
    for i in range(n):
        for j in range(i+1,n): l=l+F.relu(F.cosine_similarity(prog[i].unsqueeze(0),prog[j].unsqueeze(0))-0.3)
    return l/(n*(n-1)/2)
def use_loss(prog, np_): avg=torch.stack(prog).mean(0)[:np_]; return math.log(np_)-(-(avg*torch.log(avg+1e-8)).sum())
...
def goal_contrastive_loss(ge, rv, all_sv):
    pos=ge(rv,rv); nv=random.choice(all_sv)
...
def nearest_neighbor(pred_vec, all_states, cache):
    ...
    for st in all_states:
        vec=cache.get(st); sim=F.cosine_similarity(pred_vec.unsqueeze(0),vec.unsqueeze(0)).item()
...
def train_cycle(model, cache, s2i, i2s, chains, all_states, all_sv, n_iters, cycle, lr):
    ...
    for it in range(n_iters):
        ...
        rv=task["result_state_vec"].to(DEVICE); gl=goal_contrastive_loss(model.goal_eval,rv,all_sv)
        pt=torch.tensor(1.0 if task["precondition_holds"] else 0.0,device=DEVICE); pl=F.binary_cross_entropy(precond_sc.view(-1)[0],pt)
        loss=(latent_loss+0.1*div_loss(prog)+0.05*use_loss(prog,np_)+0.1*novelty_loss(prog,np_)+0.03*stop_l+0.02*len(prog)+cfg["goal_loss_weight"]*gl+cfg["precondition_loss_weight"]*pl)/cfg["grad_accumulation"]
        loss.backward()
        with torch.no_grad():
    ...
    with torch.no_grad():
        for rule in sorted(TRANSITION_RULES.keys()):
            ...
            for _ in range(ne):
                ...
                ps,sim=nearest_neighbor(pv,all_states,cache); ok+=ps==t["expected"]
                if shown<1: print(f"    [{rule:12s}] '{t['state'][:20]}' + '{t['action']:10s}' -> {'V' if ps==t['expected'] else 'X'} (sim:{sim:.3f})"); shown+=1
            a=100*ok/ne; results[rule]=a; print(f"      -> {a:.0f}%")
    print("\n  --- Summary ---")
    for r,a in sorted(results.items(),key=lambda x:-x[1]): print(f"    {r:12s}: {a:5.1f}% [{chr(9608)*int(a/5)}]")
    ov=sum(results.values())/len(results); print(f"\n    SINGLE-STEP (NN): {ov:.1f}%")
    print("\n  --- Latent Similarity ---")
    ts,ns_=0,0
    with torch.no_grad():
        for rule in sorted(TRANSITION_RULES.keys()):
            sims=[]
            for _ in range(10):
                t=generate_single_task(cache,s2i,rule_name=rule); pv,_,_,_,_,_,_,_,_,_=model(t,0.1,use_mem=False)
                tv=t["target_vec"].to(DEVICE); sims.append(F.cosine_similarity(pv.unsqueeze(0),tv.unsqueeze(0)).item())
            avg=sum(sims)/len(sims); ts+=avg; ns_+=1; print(f"    {rule:12s}: cos_sim={avg:.3f}")
    print(f"    AVERAGE: {ts/ns_:.3f}")
    for depth in [2,3]:
        if not chains.get(depth): continue
        print(f"\n  --- Depth-{depth} ---"); ok,tested=0,0
        with torch.no_grad():
            for _ in range(min(30,len(chains[depth]))):
                ...
                ps,sim=nearest_neighbor(pv,all_states,cache); ok+=ps==t["expected"]; tested+=1
                if tested<=2: print(f"    '{t['state'][:20]}' +[{t['action'][:20]}] -> {'V' if ps==t['expected'] else 'X'} (sim:{sim:.3f})")
...
def main():
    ...
    for c in range(cfg["num_cycles"]):
        ...
        train_cycle(model, cache, s2i, i2s, chains, all_states, all_sv, cfg["iters_per_cycle"][c], c+1, cfg["lr_per_cycle"][c])
        print(f"\n--- Eval Cycle {c+1} ---")
        evaluate(model, cache, s2i, i2s, chains, all_states)
        print(f"\n  Memory: {model.mem.stats()}")
        if c < cfg["num_cycles"] - 1:
            print(f"\n--- Compression ---"); n = model.compress()
